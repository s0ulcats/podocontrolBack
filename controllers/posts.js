...
export const createPost = async (req, res) => {
    try {
        ...
        if (req.files) {
            let fileName = Date.now().toString() + req.files.image.name; // Исправлена ошибка
            ...
            const newPostWithImage = new Post({
                ...
                title,
                text,
                imgUrl: fileName, // Передаем имя файла
                author: req.userId,
            });

            await newPostWithImage.save();
            user.posts.push(newPostWithImage._id); // Обновляем список постов у пользователя
            await user.save();
        ...
        const newPostWithoutImage = new Post({
            ...
            title,
            text,
            imgUrl: '', // Оставляем пустое изображение
            author: req.userId,
        });

        await newPostWithoutImage.save();
        user.posts.push(newPostWithoutImage._id); // Обновляем список постов у пользователя
        await user.save();
...
export const removePost = async ( req, res) => {
    try {
        ...
        if(!post) return res.send({mesage: 'Post not exist'})
